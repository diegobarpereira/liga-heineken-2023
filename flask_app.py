...
from cartolafc.constants import rodadas_campeonato, rodadas_primeiro_turno, rodadas_segundo_turno, \
    ...
    rodadas_quartas_seg_turno, rodadas_semis_seg_turno, rodadas_finais_seg_turno, premios, dict_matamata
...
def oitavas_de_final_prim_turno():
    ...
    if len(dict_matamata_oitavas['oitavas']) == 0:
        ...
        list_oitavas_prim_turno = dict_matamata_oitavas['oitavas']
    ...
    for rod_oit in rodadas_oitavas_prim_turno:
...
def quartas_de_final_prim_turno():
    ...
    if len(dict_matamata_quartas['quartas']) == 0:
        ...
        list_quartas_prim_turno = dict_matamata_quartas['quartas']
    ...
    for rod_qua in rodadas_quartas_prim_turno:
...
def get_class_liberta_seg_turno():
    jogos_rodada_21, jogos_rodada_22, jogos_rodada_23, jogos_rodada_24, jogos_rodada_25, jogos_rodada_26, jogos_rodada_27, jogos_rodada_28, jogos_rodada_29, jogos_rodada_30, g1, g2, g3, g4, g5 = get_liberta_seg_turno()
...
def oitavas_de_final_seg_turno():
    ...
    with open('static/dict_matamata_seg_turno.json', encoding='utf-8', mode='r') as currentFile:
        data_matamata = currentFile.read().replace('\n', '')
    ...
    if len(dict_matamata_oitavas['oitavas']) == 0:
        ...
        with open(f'static/dict_matamata_seg_turno.json', 'w') as f:
...
def quartas_de_final_seg_turno():
    ...
    if 33 <= rod < 35 and api.mercado().status.nome == 'Mercado fechado':
        ...
        for key, value in dict_quartas_pts.items():
            quartas.append([key,
                            value[0],
                            value[1][2] if rod == 33 else value[1][0],
                            value[1][2] if rod == 34 else value[1][1]])

    elif rod >= 35 or api.mercado().status.nome == 'Mercado aberto':
